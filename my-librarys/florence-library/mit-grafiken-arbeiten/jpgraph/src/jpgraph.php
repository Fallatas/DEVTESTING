...
//
function GenImgName() {
    // Determine what format we should use when we save the img
...
class Graph {
    ...
    // Build the argument string to be used with the csim img
    ...
    function StrokeCSIM($aScriptName='auto',$aCSIMName='',$aBorder=0) {
        ...
        if( empty($_GET[_CSIM_DISPLAY]) ) {
            ...
            if( $this->csimcachename != '' ) {
                ...
                if( $aScriptName=='' ) {
                    JpGraphError::RaiseL(25030);//('Missing js name in call to StrokeCSIM(). You must specify the name of the actual image js as the first parameter to StrokeCSIM().');
                }
