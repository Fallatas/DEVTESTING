...
HTTP headers have already been sent.<br>Caused by output from file <b>%s</b> at line <b>%d</b>.</td></tr><tr><td><b>Explanation:</b><br>HTTP headers have already been sent back to the browser indicating the data as text before the library got a chance to send it\'s image HTTP header to this browser. This makes it impossible for the library to send back image data to the browser (since that would be interpretated as text by the browser and show up as junk text).<p>Most likely you have some text in your js before the call to <i>Graph::Stroke()</i>. If this texts gets sent back to the browser the browser will assume that all data is plain text. Look for any text, even spaces and newlines, that might have been sent back to the browser. <p>For example it is a common mistake to leave a blank line before the opening "<b>&lt;?php</b>".</td></tr></table>',2),
...
12001 => array("You are using GD 2.x and are trying to use a background img on a non truecolor image. To use background img with GD 2.x it is necessary to enable truecolor by setting the USE_TRUECOLOR constant to TRUE. Due to a bug in GD 2.0.1 using any truetype fonts with truecolor img will result in very poor quality fonts.",0),
...
15011 => array('In order to use image transformation you must include the file jpgraph_imgtrans.php in your js.',0),
...
25017 => array('You are using GD 2.x and are trying to use a background img on a non truecolor image. To use background img with GD 2.x it is necessary to enable truecolor by setting the USE_TRUECOLOR constant to TRUE. Due to a bug in GD 2.0.1 using any truetype fonts with truecolor img will result in very poor quality fonts.',0),
...
25030 => array('Missing js name in call to StrokeCSIM(). You must specify the name of the actual image js as the first parameter to StrokeCSIM().',0),
...
25109 => array('Your PHP (and GD-lib) installation does not appear to support any known graphic formats. You need to first make sure GD is compiled as a module to PHP. If you also want to use JPEG img you must get the JPEG library. Please see the PHP docs for details.',0),
